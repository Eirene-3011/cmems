...
import { FiPlus, FiEdit2, FiTrash2, FiX, FiUsers, FiUser, FiArrowRight, FiActivity, FiLayers } from 'react-icons/fi';
...
export default function Ministries() {
  ...
  async function save(form) {
    try {
      ...
      toast.success('Ministry saved successfully.'); setModal(null); load();
    } catch (err) { toast.error(err.response?.data?.message || 'Error saving ministry.'); }
  }

  async function remove(id) {
    if (!confirm('Are you sure you want to delete this ministry? This action cannot be undone.')) return;
    try { await api.delete(`/ministries/${id}`); toast.success('Ministry deleted.'); load(); }
    catch (err) { toast.error(err.response?.data?.message || 'Error deleting ministry.'); }
  }
  ...
  return (
    <div className="cmems-page" style={{ minHeight:'100vh', background: C.obsidian, margin:'-24px', padding:'40px 40px 80px 40px', position:'relative', overflow:'hidden', fontFamily:'"Inter", system-ui, sans-serif' }}>
      ...
      <div style={{ position:'relative', zIndex:1, maxWidth:1280, margin:'0 auto' }}>
        ...
        <header style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', marginBottom:48, gap:24, flexWrap:'wrap' }}>
          <div>
            ...
            <h1 style={{ fontSize:36, fontWeight:900, color: C.text, margin:0, letterSpacing:'-0.02em' }}>
              Church <span style={{ color: C.sky }}>Ministries</span>
            </h1>
          </div>
          {isAdmin && (
        ...
        {loading ? (
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'120px 0' }}>
            ...
            <p style={{ marginTop:20, color: C.sub, fontWeight:500 }}>Organizing ministries...</p>
          </div>
        ) : (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(340px,1fr))', gap:24 }}>
            {ministries.length === 0 && (
              <div style={{ gridColumn:'1/-1', textAlign:'center', padding:'100px 0', background:C.panel, borderRadius:24, border:`1px dashed ${C.border2}` }}>
                <FiLayers size={48} color={C.muted} style={{ marginBottom:16, opacity:0.3 }} />
                <p style={{ color: C.muted, fontSize:16 }}>No ministries found in the database.</p>
              </div>
            )}
            {ministries.map((m, i) => (
              <div key={m.id} className="ministry-card" style={{ 
                ...
                <div style={{ position:'absolute', top:-40, right:-40, width:140, height:140, borderRadius:'50%', background:`radial-gradient(circle, ${ICON_GRADIENTS[i%6].split(',')[1].replace(')','').trim()}12, transparent 70%)`, filter:'blur(30px)', pointerEvents:'none' }} />
                
                <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:20 }}>
                  <div style={{ 
                    width:52, height:52, borderRadius:14, background: ICON_GRADIENTS[i%6], 
                ...
                <div style={{ flex:1 }}>
                  <h3 style={{ color: C.text, fontWeight:800, fontSize:18, marginBottom:8, letterSpacing:'-0.01em' }}>{m.name}</h3>
                  <p style={{ color: C.sub, fontSize:14, marginBottom:24, lineHeight:1.6, display:'-webkit-box', WebkitLineClamp:3, WebkitBoxOrient:'vertical', overflow:'hidden' }}>
                    {m.description || 'Dedicated to serving the community through specialized outreach and spiritual growth programs.'}
                  </p>
                </div>
                ...
                <div style={{ 
                  ...
                  <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                    <div style={{ width:32, height:32, borderRadius:'50%', background:C.lift, display:'flex', alignItems:'center', justifyContent:'center', color:C.sky }}>
                      <FiUser size={14} />
                    </div>
                    <div>
                      <div style={{ fontSize:10, color: C.muted, fontWeight:800, textTransform:'uppercase', letterSpacing:'.05em' }}>Leader</div>
                      <div style={{ fontSize:13, color: C.text, fontWeight:700 }}>{m.leader_name||'Not Assigned'}</div>
                    </div>
                  </div>
                  <div style={{ textAlign:'right' }}>
                    <div style={{ fontSize:10, color: C.muted, fontWeight:800, textTransform:'uppercase', letterSpacing:'.05em' }}>Capacity</div>
                    <div style={{ fontSize:13, color: C.sky, fontWeight:800 }}>{m.member_count} <span style={{ fontSize:11, fontWeight:600, color:C.sub }}>Members</span></div>
                  </div>
                </div>
      ...
      {modal && <MinistryModal form={modal} members={members} onSave={save} onClose={() => setModal(null)} />}
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(4,7,13,.8)', backdropFilter:'blur(8px)', zIndex:100, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div style={{ background: C.panel, border:`1px solid ${C.border2}`, borderRadius:24, width:'100%', maxWidth:480, boxShadow:'0 32px 80px rgba(0,0,0,.8)', overflow:'hidden' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'24px 32px', borderBottom:`1px solid ${C.border}` }}>
          <div>
            <h3 style={{ color: C.text, fontSize:20, fontWeight:800, margin:0 }}>{form.id ? 'Edit Ministry' : 'New Ministry'}</h3>
            <p style={{ color: C.sub, fontSize:12, margin:'4px 0 0 0' }}>Define the purpose and leadership of this department</p>
          </div>
        ...
        <div style={{ padding:32, display:'flex', flexDirection:'column', gap:20 }}>
          <div>
            ...
            <input style={inputStyle} placeholder="e.g. Music & Worship" value={form.name} onChange={e=>set({name:e.target.value})} />
          </div>
          
          <div>
            <label style={labelStyle}>Description</label>
            <textarea style={{ ...inputStyle, resize:'vertical', minHeight:100 }} placeholder="Briefly describe the ministry's mission..." value={form.description||''} onChange={e=>set({description:e.target.value})} />
          </div>
          
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
            <div>
              <label style={labelStyle}>Assigned Leader</label>
              <select style={{ ...inputStyle, cursor:'pointer' }} value={form.leader_id||''} onChange={e=>set({leader_id:e.target.value||null})}>
        ...
        <div style={{ padding:'20px 32px', background: C.navy, borderTop:`1px solid ${C.border}`, display:'flex', justifyContent:'flex-end', gap:12 }}>
          ...
          <button onClick={() => onSave(form)} style={{ padding:'10px 28px', borderRadius:10, border:'none', background:`linear-gradient(135deg,#1a4fa0,${C.azure})`, color:'#fff', fontSize:14, cursor:'pointer', fontWeight:700, boxShadow:`0 8px 20px rgba(37,99,235,.3)` }}>
            {form.id ? 'Update Ministry' : 'Create Ministry'}
          </button>
        </div>
      </div>
    </div>
